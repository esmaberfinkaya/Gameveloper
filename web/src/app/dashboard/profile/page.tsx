                ...
                {error ? (
                  ...
                  activeList.map((item: any) => {
                    if (activeFilter === 'issues') {
                      ...
                      return <ProjectCard key={`proj-${item.id}`} {...item} author={{ id: user.id, name: user.name, role: user.role }} currentUser={user} onUpdate={() => fetchUserData(user.id)} onClick={() => { setSelectedProject(item); setIsProjectModalOpen(true); }} />;
                    }
                    return null;
                  })
                )}
